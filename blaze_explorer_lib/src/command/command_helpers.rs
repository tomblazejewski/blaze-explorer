...
macro_rules! match_popup_call {
    ...
    ($app:ident, $func:ident, $($args:expr_2021),*) => {
        match &mut $app.popup {
            None => None,
            Some(ref mut popup) => {
                popup.$func($($args),*);
    ...
    ($app:ident, $func:ident -> $ret:ty) => {
        match &mut $app.popup {
            None => None,
            &mut Some(ref mut popup) => popup.$func(),
        }
    };

    // Case where the function has a return value
    ($app:ident, $func:ident, $($args:expr_2021),*; -> $ret:ty) => {
        match &mut $app.popup {
            None => None,
            &mut Some(ref mut popup) => popup.$func($($args),*),
        }
    };
}
#[macro_export]
